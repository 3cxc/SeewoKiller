...
extern "C" {
	...
	// 遍历系统中符合指定名称的进程
	_declspec(dllexport) void traversalProcesses(const wchar_t* targetProcessName);
	// 提供访问 processID 的函数
	_declspec(dllexport) DWORD* getProcessIDsData();
	_declspec(dllexport) size_t getProcessIDsSize();
}
